...
export const POST = withGuard(
  async (request: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    const { sopCost, price } = sopSchema.parse(await request.json());

    const vehicle = await prisma.vehicle.findUnique({
      where: { id },
      select: { id: true, status: true, costing: { select: { sopCost: true } } },
    });
    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (isFirstSet) {
        await tx.vehicle.update({ where: { id }, data: { status: "SOP_ADDED" } });
    ...
    return ok({ sopCost, transitioned: isFirstSet });
  },
);
